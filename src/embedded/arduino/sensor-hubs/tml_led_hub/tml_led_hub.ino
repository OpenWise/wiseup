...
sensor_t mapping[] = {
  { DIGITAL_TEMPERATURE_ADDR,  DIGITAL_TEMPERATURE_PIN,  TEMPERATURE_SENSOR_TYPE, 0 , 30},
  { ANALOG_LDR_ADDR,           ANALOG_LDR_PIN,           LUMINANCE_SENSOR_TYPE,   0 , 30},
  { DIGITAL_PIR_ADDR,          DIGITAL_PIR_PIN,          PIR_SENSOR_TYPE,         0 , 5},
  { DIGITAL_RELAY_ADDR,        DIGITAL_RELAY_PIN,        RELAY_SENSOR_TYPE,       0 , 5}
};
  ...
  switch (device_context.state) {
    ...
    case CONNECTED:
      ...
      if (device_context.is_sync == YES) {
        ...
        Serial.print (" ");
        Serial.print ((uint8_t)device_context.mapping_ptr[DIGITAL_PIR_ADDR - 1].value);
        Serial.print (" ");
        Serial.print ((uint8_t)device_context.mapping_ptr[DIGITAL_RELAY_ADDR - 1].value);
        Serial.println ();
      }
    break;
  }
