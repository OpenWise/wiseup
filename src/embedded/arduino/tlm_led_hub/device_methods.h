...
send_sensors_data (device_context_t * context, WiseRFComm* network) {
  ...
  for (int i = 0; i < context->mapping_size; i++) {
    ...
    if (sensor_info->flags.is_value_cng == YES) {
      ...
      *data_ptr = (uint8_t)sensor_info->value;
      ...
      sensor_info->flags.is_value_cng = NO;
    }
  }
...
void
send_keep_alive (device_context_t * context, WiseRFComm* network) {
  Serial.println ("(TLM)# Sending KEEPALIVE");
  
  rfcomm_data*         nrfTXPacket    = (rfcomm_data *)network->tx_ptr;
  rfcomm_device_prot*  protocolArea   = (rfcomm_device_prot *)nrfTXPacket->data_frame.unframeneted.data;
  ...
  protocolArea->device_cmd = DEVICE_PROT_CONNECT_CHK;
...
check_sensors_change (device_context_t * context) {
  ...
  for (int i = 0; i < context->mapping_size; i++) {
    ...
    if (context->mapping_ptr[i].value != context->mapping_ptr[i].backup) {
      context->mapping_ptr[i].flags.is_value_cng = YES;
    } else {
      context->mapping_ptr[i].flags.is_value_cng = NO;
    }
  }
